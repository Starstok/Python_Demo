...
nasip="61.146.20.254"                   #net auth server ip
wifi="1050"
url="http://enet.10000.gd.cn:10001/client/"
login = url + "login"
challenge = url + "vchallenge"
...
def chall_http_post():
        timestr=str(GetTime())
        str1=str(str(25)+client+nasip+mac+timestr+dianxin)
        md5str=getmd5(str1).upper()
        print Now_time()+"MD5>>>>>>>>>"+md5str
        url=challenge
        values ={"version":"25","username":user,"clientip":client,"nasip":nasip,"mac":mac,"timestamp":timestr,"authenticator":md5str}
